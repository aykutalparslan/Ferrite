...
using Moq;
using Xunit;
using CodeSettings = Ferrite.Data.Auth.CodeSettings;
...
public class AuthTests
{
    ...
    public async Task BindTempAuthKey_Returns_True()
    {
        var builder = GetBuilder();
        var auth = new Mock<IAuthService>();
        auth.Setup(x => x.BindTempAuthKey(It.IsAny<long>(), It.IsAny<long>(), 
    ...
    public async Task ImportBotAuthorization_Returns_Authorization()
    {
        var builder = GetBuilder();
        var authStub = new Mock<IAuthService>();
        authStub.Setup(x => x.ImportBotAuthorization(It.IsAny<int>(), It.IsAny<string>(), It.IsAny<string>()))
            .ReturnsAsync(() =>
                new Data.Auth.Authorization()
                {
                    AuthorizationType = AuthorizationType.Authorization,
    ...
    public async Task ImportBotAuthorization_Returns_SignUpRequired()
    {
        var builder = GetBuilder();
        var auth = new Mock<IAuthService>();
        auth.Setup(x => x.ImportBotAuthorization(It.IsAny<int>(), It.IsAny<string>(), 
            It.IsAny<string>())).ReturnsAsync(() => new Data.Auth.Authorization()
        {
            AuthorizationType = AuthorizationType.SignUpRequired
        });
    ...
    public async Task CheckPassword_Returns_Authorization()
    {
        var builder = GetBuilder();
        var authStub = new Mock<IAuthService>();
        authStub.Setup(x => x.CheckPassword(It.IsAny<bool>(), It.IsAny<long>(), It.IsAny<byte[]>(), It.IsAny<byte[]>()))
            .ReturnsAsync(() =>
                new Data.Auth.Authorization()
                {
                    AuthorizationType = AuthorizationType.Authorization,
    ...
    public async Task RequestPasswordRecovery_Returns_PasswordRecovery()
    {
        var builder = GetBuilder();
        var authStub = new Mock<IAuthService>();
        authStub.Setup(x => x.RequestPasswordRecovery())
            .ReturnsAsync(() =>
                "asdfasdfasdf"
            );
        builder.RegisterMock(authStub);
    ...
    public async Task RecoverPassword_Returns_Authorization()
    {
        var builder = GetBuilder();
        var authStub = new Mock<IAuthService>();
        authStub.Setup(x => x.RecoverPassword(It.IsAny<string>(), It.IsAny<PasswordInputSettings>()))
            .ReturnsAsync(() =>
                new Data.Auth.Authorization()
                {
                    AuthorizationType = AuthorizationType.Authorization,
