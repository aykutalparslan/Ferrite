...
public readonly unsafe struct MsgsStateInfo : ITLObjectReader, ITLSerializable
{
    private readonly byte* _buff;
    private readonly IMemoryOwner<byte>? _memoryOwner;
    private MsgsStateInfo(Span<byte> buffer, IMemoryOwner<byte> memoryOwner)
    {
        _buff = (byte*)Unsafe.AsPointer(ref buffer[0]);
        Length = buffer.Length;
        _memoryOwner = memoryOwner;
    }
    internal MsgsStateInfo(byte* buffer, in int length, IMemoryOwner<byte> memoryOwner)
    {
        _buff = buffer;
        Length = length;
        _memoryOwner = memoryOwner;
    ...
    public static ITLSerializable? Read(Span<byte> data, in int offset, out int bytesRead)
    {
        ...
        if(*(int*)ptr == unchecked((int)0x04deb57d))
        {
    ...
    public static unsafe ITLSerializable? Read(byte* buffer, in int length, in int offset, out int bytesRead)
    {
        ...
        if(*(int*)buffer == unchecked((int)0x04deb57d))
        {
