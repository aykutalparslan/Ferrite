...
public readonly unsafe struct bad_server_salt : ITLObjectReader, ITLSerializable
{
    private readonly byte* _buff;
    private readonly IMemoryOwner<byte>? _memoryOwner;
    private bad_server_salt(Span<byte> buffer, IMemoryOwner<byte> memoryOwner)
    {
        _buff = (byte*)Unsafe.AsPointer(ref buffer[0]);
        Length = buffer.Length;
        _memoryOwner = memoryOwner;
    }
    private bad_server_salt(byte* buffer, in int length, IMemoryOwner<byte> memoryOwner)
    {
        _buff = buffer;
        Length = length;
        _memoryOwner = memoryOwner;
    ...
    public static ITLSerializable? Read(Span<byte> data, in int offset, out int bytesRead)
    {
        ...
        var obj = new bad_server_salt(data.Slice(offset, bytesRead), null);
        return obj;
    }
    public static ITLSerializable? Read(byte* buffer, in int length, in int offset, out int bytesRead)
    {
        ...
        var obj = new bad_server_salt(buffer + offset, bytesRead, null);
        return obj;
    }
    ...
    public static bad_server_salt Create(long bad_msg_id, int bad_msg_seqno, int error_code, long new_server_salt, MemoryPool<byte>? pool = null)
    {
    ...
    public ref readonly long new_server_salt => ref *(long*)(_buff + GetOffset(4, _buff, Length));
    private void Set_new_server_salt(in long value)
    {
