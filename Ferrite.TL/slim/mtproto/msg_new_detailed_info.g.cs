...
public readonly unsafe struct msg_new_detailed_info : ITLObjectReader, ITLSerializable
{
    private readonly byte* _buff;
    private readonly IMemoryOwner<byte>? _memoryOwner;
    private msg_new_detailed_info(Span<byte> buffer, IMemoryOwner<byte> memoryOwner)
    {
        _buff = (byte*)Unsafe.AsPointer(ref buffer[0]);
        Length = buffer.Length;
        _memoryOwner = memoryOwner;
    }
    private msg_new_detailed_info(byte* buffer, in int length, IMemoryOwner<byte> memoryOwner)
    {
        _buff = buffer;
        Length = length;
        _memoryOwner = memoryOwner;
    ...
    public static ITLSerializable? Read(Span<byte> data, in int offset, out int bytesRead)
    {
        ...
        var obj = new msg_new_detailed_info(data.Slice(offset, bytesRead), null);
        return obj;
    }
    public static ITLSerializable? Read(byte* buffer, in int length, in int offset, out int bytesRead)
    {
        ...
        var obj = new msg_new_detailed_info(buffer + offset, bytesRead, null);
        return obj;
    }
    ...
    public static msg_new_detailed_info Create(long answer_msg_id, int bytes, int status, MemoryPool<byte>? pool = null)
    {
        var length = GetRequiredBufferSize();
        var memory = pool != null ? pool.Rent(length) : MemoryPool<byte>.Shared.Rent(length);
        var obj = new msg_new_detailed_info(memory.Memory.Span[..length], memory);
        obj.SetConstructor(unchecked((int)0x809db6df));
    ...
    public ref readonly long answer_msg_id => ref *(long*)(_buff + GetOffset(1, _buff, Length));
    private void Set_answer_msg_id(in long value)
    {
        var p = (long*)(_buff + GetOffset(1, _buff, Length));
        *p = value;
    }
    public ref readonly int bytes => ref *(int*)(_buff + GetOffset(2, _buff, Length));
    private void Set_bytes(in int value)
    {
        var p = (int*)(_buff + GetOffset(2, _buff, Length));
        *p = value;
    }
    public ref readonly int status => ref *(int*)(_buff + GetOffset(3, _buff, Length));
