...
public readonly ref struct msg_new_detailed_info
{
    private readonly Span<byte> _buff;
    public msg_new_detailed_info(Span<byte> buff)
    {
        _buff = buff;
    }
    ...
    public static msg_new_detailed_info Create(long answer_msg_id, int bytes, int status, out IMemoryOwner<byte> memory, MemoryPool<byte>? pool = null)
    {
        var length = GetRequiredBufferSize();
        memory = pool != null ? pool.Rent(length) : MemoryPool<byte>.Shared.Rent(length);
        memory.Memory.Span.Clear();
        var obj = new msg_new_detailed_info(memory.Memory.Span[..length]);
    ...
    public readonly long answer_msg_id => MemoryMarshal.Read<long>(_buff[GetOffset(1, _buff)..]);
    private void Set_answer_msg_id(long value)
    {
        MemoryMarshal.Write(_buff[GetOffset(1, _buff)..], ref value);
    }
    public readonly int bytes => MemoryMarshal.Read<int>(_buff[GetOffset(2, _buff)..]);
    private void Set_bytes(int value)
    {
        MemoryMarshal.Write(_buff[GetOffset(2, _buff)..], ref value);
    }
    public readonly int status => MemoryMarshal.Read<int>(_buff[GetOffset(3, _buff)..]);
