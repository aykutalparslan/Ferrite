...
public readonly unsafe struct msgs_ack : ITLObjectReader, ITLSerializable
{
    private readonly byte* _buff;
    private readonly IMemoryOwner<byte>? _memoryOwner;
    private msgs_ack(Span<byte> buffer, IMemoryOwner<byte> memoryOwner)
    {
        _buff = (byte*)Unsafe.AsPointer(ref buffer[0]);
        Length = buffer.Length;
        _memoryOwner = memoryOwner;
    }
    private msgs_ack(byte* buffer, in int length, IMemoryOwner<byte> memoryOwner)
    {
        _buff = buffer;
        Length = length;
        _memoryOwner = memoryOwner;
    ...
    public MsgsAck GetAsMsgsAck()
    {
        return new MsgsAck(_buff, Length, _memoryOwner);
    }
    ...
    public static ITLSerializable? Read(Span<byte> data, in int offset, out int bytesRead)
    {
        ...
        var obj = new msgs_ack(data.Slice(offset, bytesRead), null);
        return obj;
    }
    public static ITLSerializable? Read(byte* buffer, in int length, in int offset, out int bytesRead)
    {
        ...
        var obj = new msgs_ack(buffer + offset, bytesRead, null);
        return obj;
    }
    ...
    public static msgs_ack Create(VectorOfLong msg_ids, MemoryPool<byte>? pool = null)
    {
        var length = GetRequiredBufferSize(msg_ids.Length);
        var memory = pool != null ? pool.Rent(length) : MemoryPool<byte>.Shared.Rent(length);
        var obj = new msgs_ack(memory.Memory.Span[..length], memory);
