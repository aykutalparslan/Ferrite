...
public readonly unsafe struct dh_gen_retry : ITLObjectReader, ITLSerializable
{
    private readonly byte* _buff;
    private readonly IMemoryOwner<byte>? _memoryOwner;
    private dh_gen_retry(Span<byte> buffer, IMemoryOwner<byte> memoryOwner)
    {
        _buff = (byte*)Unsafe.AsPointer(ref buffer[0]);
        Length = buffer.Length;
        _memoryOwner = memoryOwner;
    }
    private dh_gen_retry(byte* buffer, in int length, IMemoryOwner<byte> memoryOwner)
    {
        _buff = buffer;
        Length = length;
        _memoryOwner = memoryOwner;
    ...
    public static ITLSerializable? Read(Span<byte> data, in int offset, out int bytesRead)
    {
        ...
        var obj = new dh_gen_retry(data.Slice(offset, bytesRead), null);
        return obj;
    }
    public static ITLSerializable? Read(byte* buffer, in int length, in int offset, out int bytesRead)
    {
        ...
        var obj = new dh_gen_retry(buffer + offset, bytesRead, null);
        return obj;
    }
    ...
    public static dh_gen_retry Create(ReadOnlySpan<byte> nonce, ReadOnlySpan<byte> server_nonce, ReadOnlySpan<byte> new_nonce_hash2, MemoryPool<byte>? pool = null)
    {
        var length = GetRequiredBufferSize();
        var memory = pool != null ? pool.Rent(length) : MemoryPool<byte>.Shared.Rent(length);
        memory.Memory.Span.Clear();
        var obj = new dh_gen_retry(memory.Memory.Span[..length], memory);
        obj.SetConstructor(unchecked((int)0x46dc1fb9));
        obj.Set_nonce(nonce);
        obj.Set_server_nonce(server_nonce);
        obj.Set_new_nonce_hash2(new_nonce_hash2);
        return obj;
    }
    ...
    public ReadOnlySpan<byte> new_nonce_hash2 => new (_buff + GetOffset(3, _buff, Length), 16);
    private void Set_new_nonce_hash2(ReadOnlySpan<byte> value)
    {
        if(value.Length != 16)
        {
            return;
