...
public readonly ref struct dh_gen_retry
{
    private readonly Span<byte> _buff;
    private readonly IMemoryOwner<byte>? _memory;
    public dh_gen_retry(ReadOnlySpan<byte> nonce, ReadOnlySpan<byte> server_nonce, ReadOnlySpan<byte> new_nonce_hash2)
    {
        ...
        SetConstructor(unchecked((int)0x46dc1fb9));
        Set_nonce(nonce);
        Set_server_nonce(server_nonce);
        Set_new_nonce_hash2(new_nonce_hash2);
    }
    public dh_gen_retry(Span<byte> buff)
    {
        _buff = buff;
    }
    ...
    public ReadOnlySpan<byte> new_nonce_hash2 => _buff.Slice(GetOffset(3, _buff), 16);
    ...
    public ref struct TLObjectBuilder
    {
        ...
        private ReadOnlySpan<byte> _new_nonce_hash2;
        public TLObjectBuilder with_new_nonce_hash2(ReadOnlySpan<byte> value)
        {
            _new_nonce_hash2 = value;
            return this;
        }
        public dh_gen_retry Build()
        {
            return new dh_gen_retry(_nonce, _server_nonce, _new_nonce_hash2);
        }
    }
