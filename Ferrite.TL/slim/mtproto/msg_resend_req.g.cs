...
public readonly unsafe struct msg_resend_req : ITLObjectReader, ITLSerializable
{
    private readonly byte* _buff;
    private readonly IMemoryOwner<byte>? _memoryOwner;
    private msg_resend_req(Span<byte> buffer, IMemoryOwner<byte> memoryOwner)
    {
        _buff = (byte*)Unsafe.AsPointer(ref buffer[0]);
        Length = buffer.Length;
        _memoryOwner = memoryOwner;
    }
    private msg_resend_req(byte* buffer, in int length, IMemoryOwner<byte> memoryOwner)
    {
        _buff = buffer;
        Length = length;
        _memoryOwner = memoryOwner;
    ...
    public MsgResendReq GetAsMsgResendReq()
    {
        return new MsgResendReq(_buff, Length, _memoryOwner);
    }
    ...
    public static ITLSerializable? Read(Span<byte> data, in int offset, out int bytesRead)
    {
        ...
        var obj = new msg_resend_req(data.Slice(offset, bytesRead), null);
        return obj;
    }
    public static ITLSerializable? Read(byte* buffer, in int length, in int offset, out int bytesRead)
    {
        ...
        var obj = new msg_resend_req(buffer + offset, bytesRead, null);
        return obj;
    }
    ...
    public static msg_resend_req Create(VectorOfLong msg_ids, MemoryPool<byte>? pool = null)
    {
        var length = GetRequiredBufferSize(msg_ids.Length);
        var memory = pool != null ? pool.Rent(length) : MemoryPool<byte>.Shared.Rent(length);
        var obj = new msg_resend_req(memory.Memory.Span[..length], memory);
        obj.SetConstructor(unchecked((int)0x7d861a08));
