...
public readonly ref struct msg_resend_req
{
    ...
    public static msg_resend_req Create(VectorOfLong msg_ids, out IMemoryOwner<byte> memory, MemoryPool<byte>? pool = null)
    {
        var length = GetRequiredBufferSize(msg_ids.Length);
        memory = pool != null ? pool.Rent(length) : MemoryPool<byte>.Shared.Rent(length);
        memory.Memory.Span.Clear();
        var obj = new msg_resend_req(memory.Memory.Span[..length]);
