...
public readonly ref struct destroy_auth_key
{
    ...
    public static destroy_auth_key Create(out IMemoryOwner<byte> memory, MemoryPool<byte>? pool = null)
    {
        var length = GetRequiredBufferSize();
        memory = pool != null ? pool.Rent(length) : MemoryPool<byte>.Shared.Rent(length);
        memory.Memory.Span.Clear();
        var obj = new destroy_auth_key(memory.Memory.Span[..length]);
