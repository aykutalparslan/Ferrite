...
public class SetClientDhParams : ITLObject, ITLMethod
{
    ...
    public SetClientDhParams(ITLObjectFactory objectFactory, IMTProtoService mtproto)
    {
    ...
    public async Task<ITLObject> ExecuteAsync(TLExecutionContext ctx)
    {
        ...
        if(constructor == TLConstructor.ClientDhInnerData)
        {
            ...
            if (existingKey == null || existingKey.Length == 0)
            {
                ...
                if (temp_auth_key)
                {
                    ...
                    _ = _mtproto.PutTempAuthKeyAsync(authKeyHash, authKeyTrimmed, new TimeSpan(0, 0, expiresIn));
                }
                else
                {
                    await _mtproto.PutAuthKeyAsync(authKeyHash, authKeyTrimmed);
                    _ = _mtproto.PutAuthKeyAsync(authKeyHash, authKeyTrimmed);
                }
        ...
        throw new TLExecutionException(String.Format("Constructor 0x{0:X} did not match the expected one.", constructor));
    }
