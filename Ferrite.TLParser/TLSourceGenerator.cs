        ...
        sb.Append(@")
    {
        ...
        for (int i = 0; i < combinator.Arguments.Count; i++)
        {
            ...
            else if (arg.TypeTerm.Identifier is "bytes" or "string")
            {
                ...
                {
                    sb.Append("(has_" + arg.Identifier + "?BufferUtils.CalculateTLBytesLength(len_" + arg.Identifier +
                              "):0)");
                }
                else
                {
                ...
                arg.TypeTerm.IsBare)
            {
                    ...
                    arg.TypeTerm.Identifier != "int256")
                {
                    sb.Append(arg.ConditionalDefinition.Identifier + "[" +
                              arg.ConditionalDefinition.ConditionalArgumentBit + "], " + arg.Identifier + ".Length");
                }
                else
                {
                    sb.Append(arg.ConditionalDefinition.Identifier + "[" +
